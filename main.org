...
#+begin_proof
Easily deducible from: $\expecti{V_j} = \sum_{n = 1}^{\infty} p_{i,j}^{(n)}$ and 
$\expecti{V_j} = f_{i,j}(1 + \expectj{V_j})$.
#+end_proof
...
*** =markovchain= package
We can propose an alternative definition of communicating property, taking $n \ge 0$ instead of $n > 0$, which has been the one used in the software from the start:
