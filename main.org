...
2. $P\onecol = \onecol$ and every fixed column vector $c$ holds $c = \lambda \onecol$ for some $\lambda \in \mathbb{R}$

As a consequence, there is a unique steady state for a regular stochastic matrix.
#+end_proposition
...
2. Since $c = Pc = P^2 c = \ldots P^n c$, taking limits $c = Wc$, but then
