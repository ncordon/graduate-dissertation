...
#+begin_definition
A Markov chain where all its states are essential and there exists only a communicating class is called
irreducible (or ergodic) Markov chain.
#+end_definition
...
$g_n = \twopartdefow{gcd(l_1, l_2)}{n = 1}{gcd(g_{n - 1}, l_{n + 1})}$
