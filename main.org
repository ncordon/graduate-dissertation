...
we will represent it as $i \longleftrightarrow j$.
#+end_definition

#+begin_proposition
$\longleftrightarrow$ is an equivalence relationship
#+end_proposition

#+begin_proof
1. Reflexivity holds by definition of $p_{i,i}^{(0)} = 1$
...
#+begin_proof
Straightforward from $\expecti{V_j} = \sum_{n = 1}^{\infty} p_{i,j}^{(n)}$ and 
$\expecti{V_j} = f_{i,j}(1 + \expectj{V_j})$.
#+end_proof

#+begin_definition
State $i\in S$ is essential if $i \goto j$ always implies $j \goto i$. We denote the set 
of essential states by $E^{\ast}$.
#+end_definiton

#+begin_proposition
If $i\in E^{\ast}$ and $i \goto j$, then $j \in E^{\ast}$
#+end_proposition
