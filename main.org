...
plenty of proposals from his side to work on. That always poses good challenges to dive in, learn and
apply new knowledge.
...
Markov chains are a mathematical area which encompasses probability, inference, algebra (more accurately graph theory),
algorithms theory (since discrete markov chains can be simulated in a computer, and therefore, we intend to simulate
them as efficiently as we can or use as big input sizes as we can, for example in markov chains with more states). All
of the above conditions make markov chains an interesting study from both the mathematical and the computer science viewpoint.


\vspace*{\fill}
\textsc{Keywords}: Markov chains, algorithms, graphs, probability, linear algebra
...
#+begin_proof
Looking at the canonic matrix from Equation \eqref{eq:irreducible-classes-matrix}, if $v$ is a steady state, then
...
To install and load the markovchain [fn::[[https://github.com/spedygiorgio/markovchain][https://github.com/spedygiorgio/markovchain]]] package, one could do (from an R terminal or inside RStudio):
