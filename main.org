...
#+begin_proof
Straightforward from Equation \eqref{eq:fij-recurrence} and the fact that $\sum{k \in S} p_{i,k} = 1$
#+end_proof
...
\begin{align*}
e_{i,j} &= \sum{n = 1}^{\infty} n f_{i,j}^{(n)} = p_{i,j} + \sum_{n = 2}^{\infty} n \sum_{k \neq j} p_{i,k} f_{k,j}^{(n-1)} \\
        &= p_{i,j} + \sum{k \neq j} p_{i,k} \sum{n = 2}^{\infty} n f_{k,j}^{(n-1)} \\
        &= p_{i,j} + \sum{k \neq j} p_{i,k} \bigg\{ \sum{n = 2}^{\infty} f_{k,j}^{(n-1)} + \sum{n = 2}^{\infty} (n-1)f_{k,j}^{(n-1)} \bigg\} \\
        &= p_{i,j} + \sum{k \neq j} p_{i,k} f_{k,j} + \sum{k \neq j} p_{i,k} e_{k,j} \\
        &\underset{\eqref{eq:fij-recurrence}}{=} 1 + \sum{k\neq j} p_{i,k} e_{k,j}
\end{align*}
#+end_proof
...
Since $\sum{k} p_{k,k} = 1$, and $0 \le f_{k,i} \le 1$, then it must hold $f_{k,i} = 1$ for all
...
\[
\sum{j \in S} \expecti{V_j} = \sum_{j \in S} \sum_{n = 1}^{\infty} p_{i,j}^{(n)} = 
...
#+begin_lemma
A recurrent state $i$ is recurrent positive iff:
...
#+begin_proposition
If $i,j$ are communicating recurrent states, they are both positive recurrent or null recurrent states.
#+end_proposition
...
And interchanging the roles of $i$ and $j$, we deduce $i$ is positive recurrent iff $j$ is.
#+end_proof
...
Therefore, we can reformualte Proposition \eqref{equiv-classes} as:
