...
\[
\sum_{j \in S} (1-s) F_{i,j}(s) \bigg\{1 + P_{j,j}(s)\bigg\} \underset{\eqref{\label{eq:Fs-Ps-relation}}}{=} 
...
$S - S^{\ast}$ to $S^{\ast}$, and $q = Q \cdot \left(\begin{array}{c}{ 1 \\ 1 \\ vdots \\ 1 }\right)$
(the row-wise sum of $Q$). And we will call $F^{\ast} = (f_{i,j}^{\ast})_{i \in S - S^{\ast}, j \in S^{\ast}}$,
