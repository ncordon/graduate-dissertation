...
unless otherwise stated, and $S = \{0, \ldots, m\}$.
...
\begin{eqnarray}
&& P(X_n = s_n, \ldots, X_{q + 1} = s_{q + 1} | X_{q} = s_{q}) = \nonumber \\
&& P(X_{q + 1} = s_{q + 1} | X_{q} = s_{q}) \cdots P(X_n = s_n | X_{n - 1} = s_{n - 1})
...
\begin{align*}
& p_{s, t}(q, n) = P(X_n = t | X_q = s),\quad s,t \in S, \quad n, q \in \mathbb{N}, n\neq q \\
...
Summing over $q + 1, \ldots, n -1$ in Equation \eqref{eq:trans-cond-probs}, we deduce:

\begin{equation}
p_{s, t}(q, n) = \sum_{s_{1} \in S} \ldots \sum_{s_{(n - q - 1)} \in S} \bigg\{ p_{s, s_{1}}(q, q + 1) \cdots p_{s_{(n - q - 1)}, t}(n - 1, n) \bigg\}
\label{eq:sum-probs}
\end{equation}
...
All the Markov chains we will consider henceforth will be discrete time-homogeneous ones and with
finite space state, and we will abbreviate it DTMC.
...
graphs $G=(S, E, P)$ where there is an edge $e = s \goto t, e \in E$ iff $p_{s,t} > 0$.
